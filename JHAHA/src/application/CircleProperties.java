...
public class CircleProperties extends Application {
    ...
    private abstract class LabelChange<T> {
        protected final T oldValue;
        protected final T newValue;

        protected LabelChange(T oldValue, T newValue) {
            this.oldValue = oldValue;
            this.newValue = newValue;
        }
        ...
        abstract void undo();
        abstract void redo();
        abstract LabelChange<T> invert();

        Optional<LabelChange<?>> mergeWith(LabelChange<?> other) {
            // don't merge changes by default
            return Optional.empty();
        }
    ...
    ///////
    private class TxTChange extends LabelChange<String> {
        ...
        @Override void redo() { System.out.println("aaa");}
        @Override TxTChange invert() { return new TxTChange(newValue, oldValue); }
        @Override void undo() { System.out.println("aaaaa");}
        @Override
        public boolean equals(Object other) {
    ...
    private final Label lb = new Label("aaaa");
    private final TextArea ttt = new TextArea("asdasd");
...

    {
        ...
        lb.textFillProperty().bind(colorPicker.valueProperty());
//        lb.textProperty().bind(ttt.textProperty());
        
        EventStream<TxTChange> TxTChanges = changesOf(lb.textProperty()).map(c -> new TxTChange(c));
...
//    private final EventStream<LabelChange<?>> changess;
//    private final UndoManager<LabelChange<?>> undooo;
//
//    {
////        lb.textProperty().bind(ttt.textProperty());
////        lb.textProperty().bind(ttt.textProperty());
//        
//        EventStream<TxTChange> TxTChanges = changesOf(lb.textProperty()).map(c -> new TxTChange(c));
//        changess = merge(TxTChanges);
//
//        undooo = UndoManagerFactory.unlimitedHistoryUndoManager(
//        		    changess, // stream of changes to observe
//                    c -> c.invert(), // function to invert a change
//                    c -> c.redo(), // function to undo a change
//                    (c1, c2) -> c1.mergeWith(c2)); // function to merge two changes
//
//        undoBtn.disableProperty().bind(undooo.undoAvailableProperty().map(x -> !x));
//        redoBtn.disableProperty().bind(undooo.redoAvailableProperty().map(x -> !x));
//        undoBtn.setOnAction(evt -> undooo.undo());
//        redoBtn.setOnAction(evt -> undooo.redo());
//        saveBtn.disableProperty().bind(undooo.atMarkedPositionProperty());
//        saveBtn.setOnAction(evt -> { save(); undooo.mark(); });
//    }
    ...
    public void start(Stage primaryStage) {
        ...
        Button ok = new Button("aaaa");
        ok.setOnAction(evt -> {this.lb.setText("memeda"); System.out.println(undoManager.getNextToUndo());});
        pane.getChildren().add(circle);
        
        HBox undoPanel = new HBox(20.0, undoBtn, redoBtn, saveBtn,lb,ttt,ok);
